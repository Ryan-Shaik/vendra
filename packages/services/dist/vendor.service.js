 ...
 */
export async function getShippingZones(vendorId) {
    try {
...
export async function updateVendorProfile(vendorId, input) {
    try {
        ...
        if (isPlaceholderSlug) {
            ...
            if (slugConflict) {
                return err('CONFLICT', 'A store with a similar name already exists. Try a more unique name.', 409);
            }
        }
 ...
 */
export async function createShippingZone(vendorId, input) {
    try {
        ...
        if (!onboarding?.profileComplete) {
            return err('FORBIDDEN', 'Complete your store profile before adding shipping zones.', 403);
        }
 ...
 */
export async function updateShippingZone(vendorId, zoneId, input) {
    try {
        const zone = await prisma.shippingZone.findUnique({
            where: { id: zoneId },
        });
        if (!zone)
            return err('NOT_FOUND', 'Shipping zone not found', 404);
 ...
 */
export async function deleteShippingZone(vendorId, zoneId) {
    try {
        const zone = await prisma.shippingZone.findUnique({
            where: { id: zoneId },
        });
        ...
        if (count <= 1) {
            return err('CONFLICT', 'You must have at least one shipping zone. Add another zone before deleting this one.', 409);
        }
 ...
 */
export async function completeShippingStep(vendorId) {
    try {
        const zoneCount = await prisma.shippingZone.count({ where: { vendorId } });
        if (zoneCount === 0) {
            return err('VALIDATION_ERROR', 'Add at least one shipping zone to continue.', 400);
        }
        await prisma.vendorOnboarding.update({
            where: { vendorId },
 ...
 */
export async function checkOnboardingComplete(vendorId) {
    try {
        const onboarding = await prisma.vendorOnboarding.findUnique({
            where: { vendorId },
        });
        if (!onboarding)
            return err('NOT_FOUND', 'Onboarding record not found', 404);
        if (onboarding.profileComplete &&
